...
public class LoanApplicationAdapter {
    ...
    @Autowired
    public LoanApplicationAdapter(LoanApplicationRepository loanApplicationRepository, UserRepository userRepository, LoanApplicationAdapter loanApplicationAdapter) {
    ...
    public LoanApplication transform(LoanRequest req){
        ...
        if (userOptional.isPresent()){
            return new LoanApplication(req.getAmount(), userOptional.get(),
                    Duration.ofDays(req.getDaystoRepay()), req.getInterestRatedouble());
        }
        else {
